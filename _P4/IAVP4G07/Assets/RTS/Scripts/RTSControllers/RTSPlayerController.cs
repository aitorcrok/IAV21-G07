...
{
    /* 
    * El controlador del jugador permite ofrecer una interfaz al jugador humano para que mande órdenes a uno de los ejércitos del juego.
    ...
    * 
    * Posibles mejoras:
    * - Que pueda haber 2 jugadores a la vez y que funcione bien con un mismo ratón (haría falta un botón para cambiar el turno o algo así).
    ...
    */
    ...
    { 
        ...
        // La unidad seleccionada (de mi ejército), para darle orden de moverse
        ...
        private void Awake()
        {
            // Aumenta el tamaño y cambia el color de fuente de letra para OnGUI (blanco para los humanos)
        ...
        // Actualiza el componente y está pendiente de detectar pulsaciones de ratón para seleccionar o mover unidades
        void Update()
        {
            // Podemos dibujar el rayo para verlo en la escena
            //Debug.DrawRay(ray.origin, ray.direction * 500, Color.yellow);

            // Si pulsamos el botón del ratón y no estamos sobre ningún elemento de la GUI
            if (Input.GetMouseButtonDown(0)) //&& !EventSystem.current.IsPointerOverGameObject()
            {
                ...
                if (hit)
                {
                    ...
                    else // Si no, en caso de tener unidad seleccionada, la mando para allá
                    {
                        if (selectedUnit != null)
                        {
